...
function generateSolvableLevel(){
  ...
  const groups=Math.ceil(target/3), tileW=57, tileH=57, rand=seededRandom(levelSeed);
...
function win(){
  ...
  if(completed<MAX_LEVEL){
    state.maxUnlocked=Math.max(state.maxUnlocked||1,next);
    state.level=next;
  }else{
    state.maxUnlocked=MAX_LEVEL;
...
function useShuffle(){if(gameOver||pickBusy)return;if(!state.pu.shuffle){buyPowerup("shuffle",100);return}state.pu.shuffle--;const alive=board.filter(t=>!t.removed),positions=alive.map(t=>({x:t.x,y:t.y,z:t.z}));shuffle(positions);alive.forEach((t,i)=>{t.x=positions[i].x;t.y=positions[i].y;t.z=positions[i].z});matchEpoch++;save();renderBoard();updatePowerups()}
function useHint(){
  if(gameOver||pickBusy)return;
  ...
  if(!groups.length){showToast("NO AUTO SHOT AVAILABLE","bad");return;}
   if(!accessibleGroups.length){showToast("NO SAFE AUTO SHOT AVAILABLE","bad");return;}
  const accessibleGroups=groups.filter(g=>g.filter(t=>!isBlocked(t)).length>=3);
  const pool=accessibleGroups;
...
window.TileTrailsGame={state,saveLocal,refreshUI:()=>{updateCoins();updatePowerups();buildPreview();buildMap();}};
